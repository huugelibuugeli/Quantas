...
void NotJoinedState::preComputation() {
    ...
    if (_peer->_readyToJoin) {
        if (!_createdChannel) {
            findDestination();
            if (!_peer) {
                std::cerr << "CRITICAL: state->_peer is null in NotJoinedState\n";
                return;
            }
            //std::cerr << "_peer address: " << _peer << ", publicId: " << _peer->publicId() << "\n";
            //std::cerr << "ATTEMPTING TO ACCESS PEER MEMBER VARIABLE DESTINATION: " << _peer->_dest.first << std::endl;
            //std::cerr << "SUCCESS\n";
            if (_peer->_dest.first != -1 && !_peer->_startedSearch) {
                std::cerr << "SENDING PATH FIND PAYLOAD\n";
                _peer->_startedSearch = true;
                _peer->_visited.insert(std::make_pair(_peer->_bootStrap, std::make_pair(0.5,0.5)));
...
void NotJoinedState::findDestination() {
    ...
    if (_peer->_readyToJoin) {
        ...
        if (newDest != _peer->_dest) {
            ...
            std::cerr << "deque size " << dequeSize << std::endl;
            std::cerr << "can return size" << std::endl;
            _peer->clearToVisit();
        ...
        if (msg["destination"][0] != -1) {
            ...
            if (0 == randMod(2)) {
                ...
                if (_peer->_index.first < msg["destination"][0] && _peer->_rightId != -1) { //&& _peer->_rightIdIndex.first > _peer->_index.first) {
                    message = _peer->buildRoutePayload(_peer->_rightId);
                    ...
                    message = _peer->buildRoutePayload(_peer->_leftId);
                    _peer->unicastTo(message, msg["from"]);
                }                    
                else if (_peer->_index.second < msg["destination"][1] && _peer->_upId != -1) { //&& _peer->_upIdIndex.second > _peer->_index.second) {
                    message = _peer->buildRoutePayload(_peer->_upId);
                    _peer->unicastTo(message, msg["from"]);
                }
                else if (_peer->_index.second > msg["destination"][1] && _peer->_downId != -1) { //&& _peer->_downIdIndex.second < _peer->_index.second) {
                    message = _peer->buildRoutePayload(_peer->_downId);
                    _peer->unicastTo(message, msg["from"]);
                }
                std::cerr << "peer " << _peer->publicId() << " is routing join message from " << msg["from"] << " to " << message["nextPeer"] << "\n";
            } 
            else {
                json message;
            ...
            if (_peer->_funds >= msg["funds"]) {
                if (randMod(2) == 1) {
                    ...
                    std::cerr << "peer " << _peer->publicId() << " is routing join message from " << msg["from"] << " to " << newRouteMsg["nextPeer"] << "\n";
                }
                else {
